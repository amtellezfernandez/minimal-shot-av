...
def _resolve_metrics_path(path: Path) -> Path:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    recursive = sorted(path.glob("**/metrics_results.txt")) + sorted(path.glob("**/metrics_results.json"))
    if recursive:
        return recursive[0]
    raise FileNotFoundError(f"No AlpaSim metrics_results.txt or metrics_results.json found under {path}")
...
def main() -> None:
    ...
    parser.add_argument("path", help="AlpaSim run directory or metrics_results.txt/json path.")
