...
def main() -> None:
    ...
    parser.add_argument("--min-severity", choices=("low", "medium", "high"), default="low", help="Minimum bookmark severity to include")
    args = parser.parse_args()

    manifest, frames = load_audit_log(args.audit_root)
    bundle = critical_event_bundle(manifest, frames, context_radius=max(0, args.context_radius), min_severity=args.min_severity)
        ...
        json.dumps(
            {
                ...
                "bookmark_count": bundle["bookmark_count"],
                "critical_frame_count": bundle["critical_frame_count"],
